  ...
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      role: 'assistant',
      content: '안녕하세요! **BiblioInsight AI** 도서 추천 어시스턴트입니다. 📚\n\n대한민국 최신 IT 및 개발, 인공지능 관련 도서 150여 권의 정보가 완벽하게 데이터베이스화되어 있습니다. 관심 있는 분야(예: *초보자 코딩 공부*, *클로드 코드 실무*, *업무자동화 엑셀*)나 특정 도서에 대해 자유롭게 질문해 보세요! 딱 맞는 맞춤 도서를 RAG 기술로 정밀 검색하여 추천해 드립니다.',
      timestamp: '방금 전'
    }
  ]);
      ...
      {loading ? (
        ...
        <div id="main-content-layout" className="flex-1 flex flex-col overflow-hidden">
          ...
          <div id="tab-viewport" className="flex-1 overflow-hidden bg-[#F8FAFC]">
            ...
            {/* TAB 1: DASHBOARD */}
            {activeTab === 'dashboard' && (
              <div id="panel-dashboard" className="h-full flex flex-col gap-4 p-4 overflow-hidden">
                {/* Metrics Grid */}
                <div className="grid grid-cols-4 gap-4 shrink-0">
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between hover:border-indigo-200 transition-all">
                    <div>
                      ...
                      <p className="text-2xl font-black text-slate-800 tracking-tight">{(stats?.totalBooks || 150).toLocaleString()}권</p>
                    </div>
                    <div className="flex items-center gap-1.5 text-[10px] text-indigo-600 font-bold mt-2">
                      <BookOpen className="w-3.5 h-3.5" /> YES24 실시간 데이터 연동
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between hover:border-indigo-200 transition-all">
                    <div>
                      ...
                      <p className="text-2xl font-black text-slate-800 tracking-tight">{(stats?.avgPrice || 22500).toLocaleString()}원</p>
                    </div>
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between hover:border-indigo-200 transition-all">
                    <div>
                      ...
                      <p className="text-2xl font-black text-slate-800 tracking-tight">{(stats?.avgSalesIndex || 11200).toLocaleString()}</p>
                    </div>
                ...
                <div className="flex-1 grid grid-cols-12 gap-4 overflow-hidden">
                  ...
                  <div className="col-span-7 bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 flex items-center overflow-hidden">
                      ...
                      <div className="flex-1 flex flex-col justify-center gap-3 pl-5 overflow-y-auto scroll-hide">
                        {(stats?.genreDistribution || []).map((entry: any, index: number) => {
                          const percentage = ((entry.value / (stats?.totalBooks || 150)) * 100).toFixed(1);
                          return (
                  ...
                  <div className="col-span-5 bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
                    <h3 className="text-xs font-black text-slate-700 mb-3 flex items-center gap-1.5 uppercase tracking-wider border-b border-slate-100 pb-2">
                      <Award className="w-4 h-4 text-indigo-600" /> 최고 인기 베스트셀러 순위
                    </h3>
                    <div className="flex-1 space-y-2.5 overflow-y-auto scroll-hide pr-1">
                      {(stats?.topSellers || []).slice(0, 5).map((book: Book, idx: number) => (
                        <div 
                          ...
                          className="flex items-center gap-3 p-2.5 bg-slate-50 hover:bg-indigo-50 border border-slate-100 hover:border-indigo-200 rounded-xl transition-all cursor-pointer group"
                          title="클릭하여 이 도서에 대해 AI에게 바로 물어보기"
                        >
                          <div className="w-7 h-7 rounded-lg bg-indigo-600 text-white font-extrabold text-xs flex items-center justify-center shrink-0 shadow-sm group-hover:scale-105 transition-all">
                            {idx + 1}
                          </div>
                          <div className="overflow-hidden flex-1">
                            <p className="text-xs font-bold text-slate-800 truncate group-hover:text-indigo-800 transition-colors">{book.title}</p>
                            <p className="text-[10px] text-slate-400 flex items-center justify-between mt-0.5">
                              <span className="truncate max-w-[150px]">{book.author} | {book.publisher}</span>
                              <span className="text-indigo-600 font-bold text-[9px] bg-indigo-50 px-1.5 py-0.5 rounded shrink-0">지수 {book.salesIndex.toLocaleString()}</span>
                            </p>
                          </div>
                        </div>
                      ))}
                ...
                <div className="w-[310px] shrink-0 border-r border-slate-200 bg-slate-50/50 p-4 flex flex-col gap-4 overflow-hidden">
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm shrink-0">
                    ...
                    <p className="text-[11px] text-slate-500 leading-relaxed">
                      이 챗봇은 단순한 생성형 AI가 아닌, 데이터베이스 내 **150권 이상의 도서 데이터**를 직접 대조·분석(RAG)하여 검증된 실시간 정보만을 매칭해 추천합니다.
                    </p>
                  </div>
